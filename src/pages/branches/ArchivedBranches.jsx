...
function ArchivedBranches() {
  ...
  const archivedBranches = branches.filter((branch) => branch.archived);
  useEffect(() => {
  ...
  return (
    <>
      ...
      />
      <PageWrapper title="شعب آرشیو شده">
        {branches.length === 0 ? (
          <span>هیچ شعبه ای یافت نشد</span>
        ) : (
          <ArchivedBranchTable data={archivedBranches} />
        )}
      </PageWrapper>
    </>
  );
